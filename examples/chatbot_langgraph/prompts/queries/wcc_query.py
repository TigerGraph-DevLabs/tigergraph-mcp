WCC_QUERY = """
CREATE OR REPLACE DISTRIBUTED QUERY tg_wcc (
  SET<STRING> v_type_set, SET<STRING> e_type_set, INT print_limit = 100,
  ...
  /*
   ...
   vertex is assigned an INT label = its component ID number.
    v_type_set: vertex types to traverse
    print_results: print JSON output
    e_type_set: edge types to traverse
    result_attribute: INT attribute to store results to
